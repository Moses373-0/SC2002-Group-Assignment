 ...
 * Acts as the Factory for creating concrete game objects (Factory Method
 * pattern).
 */
public class GameManager {
  ...
  public void start() {
    ...
    while (running) {
      ...
      // Create actions list (OCP: add new actions here without modifying
      // BattleEngine)
      List<Action> actions = createActions();

      // Display game info
      ui.displayBattleStart(player, level.getDifficulty());
   ...
   * To add a new action, simply add it here — BattleEngine needs no changes
   * (OCP).
   */
  ...
  private Level createLevel(int choice) {
    switch (choice) {
      case 1:
        return new Level(Difficulty.EASY);
      case 2:
        return new Level(Difficulty.MEDIUM);
      case 3:
        return new Level(Difficulty.HARD);
      default:
        return new Level(Difficulty.EASY);
    }
  }
  ...
  private TurnOrderStrategy createTurnOrderStrategy(int choice) {
    switch (choice) {
      case 1:
        return new SpeedBasedTurnOrder();
      case 2:
        return new PlayerFirstTurnOrder();
      default:
        return new SpeedBasedTurnOrder();
    }
  }
